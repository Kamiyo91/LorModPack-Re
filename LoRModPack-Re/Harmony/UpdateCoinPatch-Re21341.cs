        ...
        public static IEnumerable<CodeInstruction> Patch(IEnumerable<CodeInstruction> codeInstructions, ILGenerator iL)
        {
			...
			for (var i = 0; i < codes.Count; i++)
			{
				if (pos0 < 0 &&
					...
					codes[i + 1].operand.Equals(AccessTools.Field(typeof(EmotionBattleTeamModel), "_emotionTotalCoinNumber")))
				{
					addedCodes0 = new List<CodeInstruction>
                    {
					new CodeInstruction(OpCodes.Ldarg_0),
					new CodeInstruction(OpCodes.Ldarg_0),
					new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(EmotionBattleTeamModel), "_emotionTotalCoinNumber")),
					new CodeInstruction(OpCodes.Ldsfld, AccessTools.Field(typeof(UpdateCoinPatch_Re21341), "_coinsToRemove")),
					new CodeInstruction(OpCodes.Sub),
					new CodeInstruction(OpCodes.Stfld, AccessTools.Field(typeof(EmotionBattleTeamModel), "_emotionTotalCoinNumber")),
				};
					pos0 = i + 2;
				}
